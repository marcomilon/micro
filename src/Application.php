...
*
* It uses the method *run()* to determine witch Controller to instanciate and witch method to execute.
* 
* How it works?
*
* The method run() recive a query string of the form r=controller/action. 
* Then it will try to instanciate a Class named *ControllerCtrl* if succefull 
* it will try to execute a method call *action* that belong to the 
...
{
    /**
    * @var object trait responsable to send the response to the browser
    */
    use Response;
    ...
    */
    ...
    /**
    * Instanciate the controller class and call the action
    *
    * Instanciate the controller class and then call the method requested 
    ...
    */
    ...
    {   
        ...
        $controller = $this->instantiateController($controllerName, $module);
    ...
    */
    ...
    {   
        if (preg_match('/^([a-zA-Z]+)\/([a-zA-Z]+)\/([a-zA-Z]+)$/', $rParameter, $matches)) {
            $module = $matches[1];
            $controller = ucfirst(strtolower($matches[2])). 'Ctrl';
            $action = $matches[3];
        } elseif (preg_match('/^([a-zA-Z]+)\/([a-zA-Z]+)$/', $rParameter, $matches)) {
    ...
    */
    private function instantiateController($controllerName, $module = '') 
    {
        try {
